...
var MapNavResultPage = React.createClass({
	...
	_initResult: function () {
	    ...
	    this._lodingShow();
	    localStorage.lng_end = this.getQuery().lng_1;
	    localStorage.lat_end = this.getQuery().lat_1;
	...
	_back: function() {
		if (this.getQuery().back_url!=null) {
			...
			var back_url = href.split('back_url=')[1];
			back_url = back_url.substr(1,back_url.length-2);
			back_url.replace("%23", "#");
			// alert(back_url);
		...
		return (						
			<div className='map_nav_result'>
				...
				<div className='header_map'  >
					<div className="header_map_back" >
						<a className="header_map_back_a">
						</a>
					</div>
					<div className="header_map_title">导航路线</div>
				</div>
